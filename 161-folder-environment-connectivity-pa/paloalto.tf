# generate ssh key to enable direct access to firewalls for manual configuration
# ssh-keygen -t rsa -f ~/.ssh/gcp-lab-key -C admin -b 2048 -q -P ""
...
data "google_compute_zones" "main" {
  project = module.project-connectivity-pa-vpc.project_id
...
module "vpc_mgmt" {
  ...
  subnets = [
    {
      subnet_name   = "${local.prefix}${var.region_primary}-mgmt"
...
module "vpc_untrust" {
  ...
  subnets = [
    {
      subnet_name   = "${local.prefix}${var.region_primary}-untrust"
...
module "vpc_trust" {
  ...
  subnets = [
    {
      subnet_name   = "${local.prefix}${var.region_primary}-trust"
...
module "autoscale" {
  ...
  network_interfaces = [
    ...
    },
    {
      ...
      create_public_ip = true // Set to true if you want to access the firewalls over the internet (not recommended).
    },
    {
